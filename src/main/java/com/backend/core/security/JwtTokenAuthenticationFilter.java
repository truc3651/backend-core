  ...
  protected void doFilterInternal(
    ...
    try {
      ...
      AuthenticationException authException =
          new AuthenticationException("Token validation failed: " + e.getMessage()) {};
      authenticationEntryPoint.commence(request, response, authException);
    }
  }
        ...
        .anyMatch(
            openPath -> {
              ...
              return openPath.getMethods().stream()
                  .anyMatch(
                      method -> {
                        AntPathRequestMatcher methodMatcher =
                            new AntPathRequestMatcher(openPath.getPattern(), method.name());
